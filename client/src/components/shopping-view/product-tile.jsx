...
function ShoppingProductTile({ product,handleGetProductsDetails,handleAddToCart }) {
  return (
    <Card className="w-full max-w-sm mx-auto ">
      <div onClick={()=>handleGetProductsDetails(product?._id)}>
        ...
        <CardFooter>
          {
            product?.totalStock === 0  ? <Button className='w-full opacity-65 cursor-not-allowed' >
                Out of stock
            </Button> : <Button className='w-full' onClick={()=>handleAddToCart(product?._id,product?.totalStock)}>
            Add to cart
        </Button>
          }
        </CardFooter>
